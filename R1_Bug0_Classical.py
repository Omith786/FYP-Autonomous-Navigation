...
Bug0 Evaluation Script
------------------------------------------------------------
Simulates a classical Bug0 navigation strategy on BasicObstacleEnv
...
as it expects a trained model from Stable-Baselines3, but Bug0 uses a hardcoded policy.

Run it with this: python R1_Bug0_Classical.py --episodes 30 --out eval_runs/Bug0
...
# Reused from R1_Evaluate_Model.py with minimal changes
...
# --- Bug0 policy: simple classical algorithm ---

def _bug0_action(env: BasicObstacleEnv) -> int:
    """
    Simple Bug0 logic: go straight toward goal unless blocked, then rotate right.
    Returns: action int (0 = forward, 1 = left, 2 = right)
    """
    sonar_center = env._ultra(env.robot_angle)
    vec = env.goal_pos - env.robot_pos
    angle_to_goal = (np.degrees(np.arctan2(vec[1], vec[0])) - env.robot_angle + 360) % 360

    if angle_to_goal > 180:
        angle_to_goal -= 360  # Normalize to [-180, 180]

    if sonar_center > 0.3:
        return 0  # Forward
    elif angle_to_goal > 5:
        return 2  # Turn right
    elif angle_to_goal < -5:
        return 1  # Turn left
    else:
        return 2  # Turn right (wall follow fallback)
...
def run_bug_eval(episodes: int, env_kwargs: dict, out_dir: Path, wire: bool):
    ...
    plt.title("Bug0 Episode Outcomes")
...
def main():
    parser = argparse.ArgumentParser("Bug0 navigation evaluation on BasicObstacleEnv")
    ...
    parser.add_argument("--out", type=Path, default="eval_runs/Bug0", help="Output directory")
